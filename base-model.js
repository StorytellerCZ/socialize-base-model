...
export class BaseModel {
  ...
  static attachSchema(schemaInstance) {
    ...
    if (collection) {
      ...
      throw new Meteor.Error(
        "Can't append schema to non existent collection. Please attach a collection to your model using `ModelName.attachCollection`"
      );
    }
  }
  ...
  getCollection() {
    ...
    if (this)
      throw new Meteor.Error(
        'noCollection',
        'You must use ClassName.attachCollection to attach a collection to your model.'
      );
  }
  ...
  getUpdatableFields() {
    ...
    for (const key of Object.keys(this)) {
      if (
        schema[key] &&
        !(schema[key].custom && schema[key].custom === SimpleSchema.denyUntrusted) &&
        !schema[key].denyUpdate
      ) {
        fields[key] = this[key];
      }
    }
